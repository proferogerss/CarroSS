...
const { calcularPagoBase, generarAmortizacion, round2 } = require('../utils/amortizacion');
...
/**
 * Nota: el IVA no cambia el saldo insoluto ni la mensualidad calculada aquí
 * (solo se suma al pago total mostrado en la vista mensual), pero se incluye
 * para que el comparativo use exactamente los mismos parámetros que
 * amortizacionController.js.
 */
...
async function eventosPreviosDeCredito(creditoId, periodoAExcluir) {
  const { rows } = await pool.query(
    `SELECT * FROM eventos_credito
     WHERE credito_id = $1
       AND NOT (tipo = 'pago_extra' AND mes = $2 AND origen = 'recalculo_semanal')
     ORDER BY mes ASC`,
    [creditoId, periodoAExcluir]
  );
  return rows;
}
 ...
 * mensualidad base. Si hubo un excedente, calcula AMBOS escenarios posibles
 * (reducir plazo / reducir mensualidad) y aplica el que venga en req.body.modo
 * (default 'plazo', igual que el comportamiento anterior), registrándolo como
 * evento de "pago extra a capital" (origen recalculo_semanal) para que la
 * tabla de amortización se recalcule con ese abono.
 */
async function recalcular(req, res) {
  ...
  const modoSolicitado = req.body?.modo === 'mensualidad' ? 'mensualidad' : 'plazo';
  ...
  if (diferencia > 0.01) {
    const params = paramsDeCredito(credito);
    const eventosPrevios = await eventosPreviosDeCredito(req.params.creditoId, periodo);

    const resultadoPlazo = generarAmortizacion(params, [
      ...eventosPrevios,
      { mes: periodo, tipo: 'pago_extra', monto: diferencia, modo: 'plazo' },
    ]);
    const resultadoMensualidad = generarAmortizacion(params, [
      ...eventosPrevios,
      { mes: periodo, tipo: 'pago_extra', monto: diferencia, modo: 'mensualidad' },
    ]);

    comparativo = {
      plazo: {
        descripcion: 'Mantiene la mensualidad base y acorta el plazo (menos total a pagar).',
        mensualidad: resultadoPlazo.pagoBase,
        plazoNuevo: resultadoPlazo.plazoReal,
        mesesQueSeAdelanta: params.plazoMeses - resultadoPlazo.plazoReal,
      },
      mensualidad: {
        descripcion: 'Mantiene el plazo y reduce la mensualidad de los meses restantes.',
        mensualidad: resultadoMensualidad.pagoBaseFinal,
        plazoNuevo: resultadoMensualidad.plazoReal,
      },
    };

    const { rows } = await pool.query(
      `INSERT INTO eventos_credito (credito_id, tipo, mes, monto, observaciones, origen, modo, created_by)
       VALUES ($1,'pago_extra',$2,$3,$4,'recalculo_semanal',$5,$6)
       RETURNING *`,
      [
        req.params.creditoId,
        periodo,
        diferencia,
        `Excedente de pagos semanales del periodo ${periodo}`,
        modoSolicitado,
        req.usuario.id,
      ]
    );
  ...
  res.json({
    ...
    sumaPagada,
    diferencia,
    modoAplicado: eventoCreado ? modoSolicitado : null,
    eventoCreado,
    comparativo,
